...
export default function AdminVocabularyPage() {
  ...
  const load = async () => {
    try {
      const data = await adminGetVocabulary();
      setItems(data);
    } catch (e) {
  ...
  const filteredItems = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      list = list.filter((v) => {
        ...
        const meaning = (v.meaning_ja ?? "").toLowerCase();
  ...
  }, [items, searchQuery, levelFilter, sortOrder]);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery, levelFilter, sortOrder]);
  ...
  return (
    <div>
      <h1 className="mb-6 text-xl font-semibold text-slate-100">単語管理</h1>
        ...
        <table className="w-full text-left text-sm">
          <thead className="border-b border-slate-700 bg-slate-800/50">
            <tr>
              ...
              <th className="px-4 py-3 font-medium text-slate-300">品詞</th>
              <th className="px-4 py-3 font-medium text-slate-300" />
            </tr>
          </thead>
          <tbody>
            {paginatedItems.map((v) => (
              ...
              >
                ...
                <td className="px-4 py-3 font-medium text-slate-200">{v.word}</td>
                <td className="px-4 py-3 text-slate-300">{v.meaning_ja}</td>
                <td className="px-4 py-3 text-slate-500 text-xs">{v.part_of_speech ?? "-"}</td>
      ...
      {filteredItems.length === 0 && (
        <p className="mt-6 text-center text-slate-500">
          {searchQuery || levelFilter ? "検索に一致する単語はありません" : "単語が登録されていません"}
        </p>
      )}
    </div>
  );
