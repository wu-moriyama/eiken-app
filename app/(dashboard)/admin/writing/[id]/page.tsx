...
export default function AdminWritingEditPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const data = await adminGetWritingPromptById(id);
        if (!data) {
          setError("問題が見つかりません");
          return;
        }
        ...
        setTimeLimitMax(data.time_limit_max_seconds != null ? String(data.time_limit_max_seconds) : "");
      } catch (e) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await adminUpdateWritingPrompt(id, {
        level,
  ...
  if (error && !title) {
    return (
      <div>
        ...
        <Link href="/admin/writing" className="text-brand-400 hover:underline">
          ← ライティング問題一覧へ
        </Link>
      </div>
    );
